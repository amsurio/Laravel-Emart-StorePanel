                ...
                <div class="restaurant_payout_create-inner">
                        ...
                        <div class="form-group row width-50">
                            ...
                            <div class="col-7">
                                <select id='vendor_cuisines' class="form-control" required>
                                    <option value="11">{{ trans('lang.select_category') }}</option>
                                </select>
                                <div class="form-text text-muted">
                    ...
                    <fieldset id="delivery_charges_div">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="form-group row width-100">
                                <label
                                    class="col-4 control-label">{{ trans('lang.minimum_delivery_charges_within_km') }}</label>
                                <div class="col-7">
...
<script>
    ...
    $(document).ready(async function() {
        ...
        ref_sections.get().then(async function(snapshots) {
            ...
            snapshots.docs.forEach((listval) => {
                ...
                if (data.serviceTypeFlag == "delivery-service" || data
        ...
        ref_deliverycharge.get().then(async function(snapshots_charge) {
            ...
            try {
                if (deliveryChargeSettings.vendor_can_modify) {
                    ...
                    $("#delivery_charges_per_km").val(deliveryChargeSettings
                        .delivery_charges_per_km);
                    $("#minimum_delivery_charges").val(deliveryChargeSettings
    ...
    function addMoreButton(day, day2, count) {
        ...
        $('#special_offer_table_' + day + ' tr:last').after('<tr>' +
            '<td class="" style="width:10%;"><input type="time" class="form-control" id="openTime' + day + count +
            '"></td>' +
            '<td class="" style="width:10%;"><input type="time" class="form-control" id="closeTime' + day + count +
            '"></td>' +
            '<td class="" style="width:30%;">' +
            '<input type="number" class="form-control" id="discount' + day + count + '" style="width:60%;">' +
            '<select id="discount_type' + day + count +
            '" class="form-control" style="width:40%;"><option value="percentage"/>%</option><option value="amount"/>' +
            currentCurrency + '</option></select>' +
            '</td>' +
            '<td style="width:30%;"><select id="type' + day + count +
            '" class="form-control"><option value="delivery"/>Delivery Discount</option></select></td>' +
            '<td class="action-btn" style="width:20%;">' +
            '<button type="button" class="btn btn-primary save_option_day_button' + day + count +
            '" onclick="addMoreFunctionButton(`' + day2 + '`,`' + day + '`,' + countAddButton +
        ...
        reader.onload = (function(theFile) {
            return function(e) {
                ...
                if (photo) {
                    ...
                    menuPhotoCount++;
                    photos_html = '<span class="image-item" id="photo_menu' + menuPhotoCount +
    ...
    async function storeGalleryImageData() {
        ...
        if (restaurnt_photos.length > 0) {
            await Promise.all(restaurnt_photos.map(async (resPhoto, index) => {
                ...
                var uploadTask = await storageRef.child(restaurant_photos_filename[index])
    ...
    async function storeMenuImageData() {
        ...
        if (vendor_menu_photos.length > 0) {
            await Promise.all(vendor_menu_photos.map(async (menuPhoto, index) => {
                ...
                var uploadTask = await storageRef.child(vendor_menu_filename[index]).putString(
                    menuPhoto, 'base64', {
                        contentType: 'image/jpg'
                    });
                var downloadURL = await uploadTask.ref.getDownloadURL();
